...
# =========================
def main():
    ap = argparse.ArgumentParser(description="LLM reproducibility benchmark (10x only)")
    ...
    reps = 10   # fixed number of repetitions
    ...
    log.info(f"Running 10x reproducibility test for presets: {presets}")
    ensure_dir(out_dir)
    ...
    for preset in presets:
        ...
        log.info(f"=== Testing '{preset}' (provider={provider}, model={model_name}) ===")
        ...
        total = len(stability_flags)
        log.info(f"\n✅ {preset} reproducible queries: {stable_count}/{total}")
